...
If this is your first time developing your own R package, welcome! You'll need/want to set up a few things, but once you do you're set to make as many packages as your heart desires! "Developing your own package" may sound scary, but the actual package development here will only involve tweaking a couple of files. I'll walk through that process below. For a clear and broader treatment of package development, [check out this awesome resource](https://r-pkgs.org/).

First things first:


  - You'll probably want git and github for version control and for deploying your package so that your teammates/labmates can use it too. If you haven't done this before, it's a bit involved to explain here, but there's an excellent walk-through: [Happy Git with R](https://happygitwithr.com/)
  - You'll need the software to compile an R package. These vary by operating system, and there's a nice summary with links [here](https://support.posit.co/hc/en-us/articles/200486498-Package-Development-Prerequisites).
  -
...
If you're developing a palette from scratch for your data set, the `palette_augment_helper()` function can again be helpful. If you don't give it a `palette.name` argument, it just compares the proposed augment to the data and identifies missing colors (plus plots the color scheme of the augment). I find this helpful for iteratively building a new palette for my data.

If you are managing your own package, the `augment2palette()` function gives step-by-step instructions for adding your new palette (in the form of an augment) to the package. This function won't change any of your code, just give you direction to simplify/streamline modifying your package.
