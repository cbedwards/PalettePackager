...
#'
#' The plotting functions don't work if your data contains taxa (or whatever identifier you're using) that aren't present in the data.
#' This identifies those mismatches, and provides guidance to look for typos and capitalization differences.s
#'
...
#'
#' @examples
#' dat = data.frame(vals = rnorm(4), species = c("acremonium", "aspergillus",
#' "candida", "vibrio"))
#' palette_check(dat$species, "wolfe2014")
#' # Now with a new taxa not in the Wolfe 2014 palette (because it's common milkweed, not a microbe).
#' dat = data.frame(vals = rnorm(5), species = c("acremonium", "aspergillus",
#' "candida", "vibrio", "Asclepias syriaca"))
