...
#' These augments can be provided directly to `scale_color_custom()` and `scale_fill_custom()`, but making them can be a bit of a bother.
#' This function helps iterate through that process. See more details in the readme.
#'
#' This function can also help develope new palettes to add to your package.
#'
...
palette_augment_helper = function(dat.names,
  ...
  stopifnot(ncol(augment)==2 | is.null(augment),
            all(names(augment)%in%c("name","color"))| is.null(augment),
